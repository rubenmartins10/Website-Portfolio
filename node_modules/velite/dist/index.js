...
export { VeliteFile, assets, build, defineCollection, defineConfig, defineLoader, defineSchema, getImageMetadata, isRelativePath, logger, processAsset, rehypeCopyLinkedFiles, remarkCopyLinkedFiles, zod_exports as z } from './chunk-4HFW4XPZ.js';
...
var require_extend = __commonJS({
  "node_modules/.pnpm/extend@3.0.2/node_modules/extend/index.js"(exports$1, module) {
...
var markdown = (options = {}) => custom((i) => i === void 0 || typeof i === "string").transform(async (value, { meta, addIssue }) => {
  ...
  try {
    const html5 = await unified().use(remarkParse).use(remarkPlugins).use(remarkRehype, { allowDangerousHtml: true }).use(rehypeMetaString).use(rehypeRaw).use(rehypePlugins).use(rehypeStringify).process({ value, path: meta.path });
...
var mdx = (options = {}) => custom((i) => i === void 0 || typeof i === "string").transform(async (value, { meta, addIssue }) => {
  value = value ?? meta.content;
  if (value == null || value.length === 0) {
    addIssue({ code: "custom", message: "The content is empty" });
    return "";
...
var metadata = () => custom((i) => i === void 0 || typeof i === "string").transform(async (value, { meta, addIssue }) => {
  value = value ?? meta.plain;
  if (value == null || value.length === 0) {
    addIssue({ code: "custom", message: "The content is empty" });
...
var path2 = (options) => custom((i) => i === void 0 || typeof i === "string").transform(async (value, { meta, addIssue }) => {
  if (value != null) {
    addIssue({ fatal: false, code: "custom", message: "`s.path()` schema will resolve the flattening path based on the file path" });
  }
  const flattened = relative(meta.config.root, meta.path).replace(/\.[^.]+$/, "").replace(/\\/g, "/");
...
// src/schemas/slug.ts
var slug = (by = "global", reserved = []) => stringType().min(3).max(200).regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/i, "Invalid slug").refine((value) => !reserved.includes(value), "Reserved slug").superRefine((value, { path: path3, meta, addIssue }) => {
...
function search(root3, expression, settings) {
  ...
  visit(root3, "heading", function(node, position, parent) {
    ...
    if ((endIndex || !expression) && (!settings.minDepth || node.depth >= settings.minDepth) && (!settings.maxDepth || node.depth <= settings.maxDepth) && (!skip || !skip.test(value))) {
...
var toc2 = (options) => custom((i) => i === void 0 || typeof i === "string").transform(
...
var unique = (by = "global") => stringType().superRefine((value, { path: path3, meta, addIssue }) => {
