    ...
    <>
      ...
      <div className="space-y-8">
        {projetos.map((projeto) => (
          <div
            key={projeto.slug}
            className="group border border-white/5 rounded-2xl bg-zinc-900/40 backdrop-blur-md hover:border-emerald-400/20 transition-all duration-300 overflow-hidden"
          >
            <div className="grid grid-cols-1 md:grid-cols-5 min-h-[380px]">
              ...
              <div className="md:col-span-3 p-8 md:p-10 flex flex-col justify-between">
                <div>
                  {/* Date tag */}
                  <span className="font-mono text-[10px] text-zinc-600 tracking-widest uppercase mb-3 block">
                    {new Date(projeto.data).toLocaleDateString("en-US", { month: "long", year: "numeric" })}
                    {projeto.destaque && (
                      <span className="ml-3 text-emerald-400 border border-emerald-400/20 bg-emerald-400/5 px-2 py-0.5 rounded-full">
                        FEATURED
                      </span>
                    )}
                  </span>
                  ...
                  {/* Title */}
                  <h2 className="font-mono font-black text-white text-xl md:text-2xl uppercase tracking-wider mb-3 leading-tight group-hover:text-emerald-400 transition-colors">
                    {projeto.nome}
                  </h2>
                  ...
                  {projeto.conquistas?.length > 0 && (
                    <div className="mb-6">
                      <div className="flex items-center gap-2 mb-3">
                        <span className="font-mono text-[10px] text-emerald-400 tracking-[0.3em] uppercase">Key Achievements</span>
                      </div>
                  ...
                  {projeto.tecnologias?.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mb-6">
                      {projeto.tecnologias.map((tech) => (
                        <span
                          key={tech}
                          className="font-mono text-[10px] text-zinc-400 border border-zinc-700/60 px-2.5 py-1 rounded tracking-wide hover:border-emerald-400/30 hover:text-emerald-400 transition-colors"
                        >
                          {tech}
                        </span>
                      ))}
                  ...
                  <div className="flex flex-wrap gap-3">
                    {projeto.url ? (
                      <a
                        ...
                        className="font-mono text-xs text-zinc-950 bg-emerald-400 hover:bg-emerald-300 px-5 py-2.5 rounded-lg font-bold tracking-widest uppercase transition-colors flex items-center gap-2"
                      >
                      ...
                      <span className="font-mono text-xs text-zinc-600 border border-white/5 px-5 py-2.5 rounded-lg tracking-widest uppercase flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="font-mono text-xs text-zinc-300 border border-white/10 hover:border-emerald-400/30 hover:text-emerald-400 px-5 py-2.5 rounded-lg tracking-widest uppercase transition-colors flex items-center gap-2"
                    >
                      Details →
                    </button>
                  </div>
              ...
              <div className="md:col-span-2 relative min-h-[220px] md:min-h-0 overflow-hidden">
                ...
                <div className="absolute inset-0 bg-linear-to-r from-zinc-900/80 via-transparent to-transparent md:bg-linear-to-r" />
              </div>
            </div>
          </div>
        ))}
